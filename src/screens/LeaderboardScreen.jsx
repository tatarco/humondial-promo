...
export default function LeaderboardScreen({ token, campaignId, onNavigateHome, onBranchBooking }) {
  ...
  const PODIUM_COL = {
    0: { left: 'calc(23% - 44px)', top: Math.max(0, Math.round(138 * podiumScale) - _hdr) },
    1: { left: '50%', transform: 'translateX(calc(-50% + 8px))', top: Math.max(0, Math.round(105 * podiumScale) - _hdr) },
    2: { left: 'calc(77% - 44px)', top: Math.max(0, Math.round(138 * podiumScale) - _hdr) },
  };
  const PODIUM_META = {
      ...
      <div className="flex-shrink-0 relative w-full" style={{ height: Math.round(220 * podiumScale) }}>
        {PODIUM_ORDER.map((rank, colIdx) => {
          ...
          return (
            ...
            >
              <div className="text-xs font-black text-center w-full" dir="auto" style={{ color: meta.textColor, textShadow: '0 2px 6px rgba(0,0,0,1)' }}>
