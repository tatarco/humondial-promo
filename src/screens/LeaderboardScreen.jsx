...
export default function LeaderboardScreen({ token, campaignId, onNavigateHome, onBranchBooking }) {
  ...
  // col positions (left→right) aligned to background frame centres
  const PODIUM_COL = {
    0: { right: '62%', top: 72 },
  ...
  return (
    <div className="min-h-dvh leaderboard-bg overflow-y-auto pb-20" dir="rtl">
      ...
      {/* Podium — content positioned inside background image frames */}
        ...
        {PODIUM_ORDER.map((rank, colIdx) => {
          ...
          return (
            ...
            >
              ...
              <div
                className="text-xs font-black text-center max-w-full truncate"
                style={{ color: meta.textColor, textShadow: '0 1px 4px rgba(0,0,0,0.8)' }}
              >
                {entry.nickname}
              </div>
              {entryTier && (
